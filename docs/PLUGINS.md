...
`collab→dict` · `xray→dict` · `quote→dict` · `price→dict` · `scan→dict`.
...
bundle/eagle/easyeda/easyeda-sch/jlc/json/kicad/kicad-sch/ocd/altium/pcad/schlib · `importer`
...
`collab` std (realtime ops: validate + apply shared-board mutations) ·
