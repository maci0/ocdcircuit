...
## Decision
DRC (`ocdcircuit/drc.py`) enforces the selected fab profile (`fab.py`;
default `jlc`: min trace/space 0.09mm, min drill 0.2mm, annular 0.15mm,
edge 0.3mm). Errors: shorts, overlaps, width, drill, floating pins, edge.
Clearance-only hits are warnings (router debt meter, ADR-0002).
