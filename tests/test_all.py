...
for _frag in ("id=edwrap", "id=pcbwrap", "id=schwrap", "id=wrap3d",
              ...
              "id=xraygo", "id=kbprefsbtn", "id=chat"):
...
_wv = open(os.path.join(HERE, "..", "apps", "web", "views.js")).read()
for _frag in ("id=filetree", "id=tree", "id=treenote", "id=importfile",
              "id=importstat", "id=drc", "id=tidy", "id=tidycov"):
...
# unknown soldermask warns (renderers silently fall back to green)
_lm = agent.loads("board t 40x30 2L\npart R1 R0805 10k\nN :: R1.1 R1.2\n", base=EX)
_lm.meta["mask"] = "gren"
...
# unknown logic= warns (gates simulator silently skips unknown kinds)
_lj = agent.loads("board t 40x30 2L\npart U1 SOIC14 logic=NANDX\nN :: U1.1 U1.2\n", base=EX)
