...
def export_jlc(board: Board, outdir: str = "out",
    ...
    # "Do not place" is per-line; never merge placed + DNP). LCSC, tol
    # and lcstat are part of the key: same value+fp with different
    # tolerance or lifecycle must not merge (JLC orders per row;
    # first-wins ships the wrong reel).
    groups: dict[tuple[str, str, str, str, str, str], list[str]] = {}
    for p in board.parts.values():
        groups.setdefault((p.value, p.fp, str(p.attrs.get("lcsc", "")),
                           str(p.attrs.get("tol", "")),
                           str(p.attrs.get("lcstat", "")),
